...
if __name__ == '__main__':
    ...
    print("[实时] 开始增强训练集 pairs")
    t0 = time.time()
    aug_pairs = augment_self_pairs(pairs, query2task)
    print(f"[实时] 训练集增强完成, 用时: {time.time()-t0:.2f}s, 增强数: {len(aug_pairs)}")
    
    print("[实时] 开始增强测试集 pairs_test")
    t0 = time.time()
    aug_pairs_test = augment_self_pairs(pairs_test, query2task)
    print(f"[实时] 测试集增强完成, 用时: {time.time()-t0:.2f}s, 增强数: {len(aug_pairs_test)}")
